# G9-12 Sentence-Skill Progression Map (VERIFIED, with assessment evidence)

**Status:** VERIFIED (source-triangulated, cell-by-cell) + assessment-backed (spiral bands grounded in the reference tests). Supersedes the provisional draft cut in the spec (`docs/superpowers/specs/2026-07-08-cross-grade-sentence-progression-and-course-arc.md` §B).
**Date:** 2026-07-08
...
## Layer 2: Genre-Specific Sentence Moves (with maturity trajectory)

| Skill | INTRODUCE @ | Maturity trajectory | Assessed capability + systems (cited) |
|-------|-------------|---------------------|----------------------------------------|
| Claim sentence (basic defensible claim) | **G9** | G9 introduce -> G10 develop (precise claim) -> G11-12 master (nuanced/sophisticated) | Crosswalk Argument CR: G9 "clear claim + reasons" -> G10 "precise claim" -> G11 "nuanced claim." |
| Attributed-evidence sentence (cite/quote/paraphrase a source) | **G9** | G9 introduce -> G10 develop (multi-source) -> G11 master (synthesis/credibility) | Crosswalk Research/Evidence rows: G9 textual evidence -> G10 integrate/cite -> G11 synthesize + evaluate credibility. |
| Counterclaim-aware claim ("Although some argue X, Y because Z") | **G10** | G10 introduce -> G11-12 develop into nuanced claim | **COUNCIL split.** Crosswalk STAAR English II "develop claim AND counterclaim fairly" @ G10; ACC.W.ARG.2. |
| Device-to-effect-to-warrant sentence (text-dependent analysis) | **G10** | G10 introduce -> G11 develop (rhetorical analysis) -> G12 master | Crosswalk G10 "analyze a text's ideas/craft using evidence" (SC-TDA, PA Keystone, MA, Regents). |
| Nuanced claim (qualification/complexity embedded in the claim) | **G11** | G11 introduce -> G12 master (AP sophistication) | **COUNCIL split.** Crosswalk G11 "nuanced claim addressing complexity" (SBAC, ACT, NY Regents, AP Lang). Winner TWR-syntax + G11 crosswalk; loser K&H G12. |
| Cross-source / synthesis sentence (connect/compare 2+ sources in one sentence) | **G11** | G11 introduce -> G12 master (advanced synthesis) | Crosswalk G11 "synthesis across sources" (SBAC explanatory, AP Lang 6-source). G10 does 2-3 source *sequential* integration (Research Simulation), not cross-source-in-one-sentence. |
| Rhetorical-analysis sentence (name an author's device/strategy + analyze its effect/purpose) | **G11** | G11 introduce -> G12 master (AP Lang/Lit) | Crosswalk G11 "rhetorical analysis of author's choices" (AP Lang, AP Lit, NH SAT essay, FL G11). Distinct from G10 device-to-effect-to-warrant. |
| AP-level sophistication moves (competing perspectives integrated, argument situated in broader context, persuasive style) | **G12** | G12 introduce/master (ceiling) | Crosswalk G12 LADDER (AP "sophisticated argument"); AP Sophistication row 0-1 (TestDesign §2). |

---

## Assessment evidence + exemplar reconstructions (per capability, per grade)
...
- **G10 DEVELOP** - format: edit-in-context MC (STAAR/TN editing; FL FAST reading-embedded). *Demand rises to parallelism + formation in a longer draft.*
  ...
  > A) NO CHANGE  B) **reducing waste, increasing recycling, and expanding transit**  C) reduce waste, increasing recycling, and to expand transit  D) reduced waste, increase recycling, expanding transit.
  > Key: B. Skill: parallelism (G10 introduce), riding on G9 combining. Same 4-option NO-CHANGE shell, harder target.

- **G11 MASTER** - format: SAT short-passage single-MC (adaptive, easiest-to-hardest) / ACT passage-based, "as a writer ... to meet rhetorical goals" (batch4). *Demand: pick the revision that is not just correct but rhetorically strongest.*
  > Reconstruction: A paragraph argues that a policy is urgent. The writer wants the final sentence to emphasize urgency by subordinating the concession. Which revision best meets that goal?
  > A) NO CHANGE (two coordinate clauses)  B) **Although the cost is real, the danger of delay is greater.**  C) The cost is real, and the danger of delay is greater.  D) The cost is real; the danger of delay is greater.
...
The crosswalk documents this row escalating G10 -> G11, which is the direct evidence for the council's precision/concision split.

- **G10 INTRODUCE (precision)** - format: edit-in-context word-choice MC (EOC "knowledge of language," VT word choice, batch1/3). *Demand: replace a vague word with a precise one; meaning-preserving.*
  > Reconstruction: "The results were good, showing a big drop in errors." Choose the most precise revision of the underlined words.
  > A) NO CHANGE  B) **strong, showing a 40 percent drop**  C) nice, showing a large drop  D) fine, showing a sizable drop.
  > Key: B (precise + concrete). Skill: basic precision, 2-3 interacting elements (the council's G10 placement).

- **G11 MASTER (concision + style)** - format: ACT Knowledge of Language / SAT Craft & Structure + Expression of Ideas, "improve effectiveness ... meet rhetorical goals" (batch4). *Demand: cut redundancy AND preserve tone/emphasis; unbounded revision.*
  > Reconstruction: "In my own personal opinion, I believe that the policy is basically and essentially unfair to students." Choose the most effective revision.
  > A) NO CHANGE  B) In my opinion, I believe the policy is unfair.  C) **The policy is unfair to students.**  D) Personally, the policy is essentially unfair.
  > Key: C (concision + directness; removes hedging without losing the claim). Skill: stylistic concision, 5-7 interacting elements (the council's G11 placement).

- **G12 MASTER (rhetorical style)** - format: AP FRQ (style as a Sophistication signal, batch4). Style/precision function as rhetorical choices sustaining the argument, not correctness edits.
...
## What changed from the spec's provisional draft
