...
- [ ] **Step 4: Full verification** — `python pipeline/tier_a_regression.py {G9,G10,G11,G12}` ALL clean (no new-gate failures remain); `python -m pytest pipeline/tests/ -q` green; re-run the S1 recognition detector + option sweeps to confirm no regressions introduced.
