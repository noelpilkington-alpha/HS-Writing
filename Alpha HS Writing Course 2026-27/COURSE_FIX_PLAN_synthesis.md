...
- **DECISION (Noel, 2026-07-17): DROP counterclaim from the G9 l27 gate + its PP100; counterclaim stays in G10.** Rule applied: it belongs where the standards/tests place it (G10) unless there is *irrefutable* evidence it earns its keep in G9 — and there is none. Evidence: counterclaim is a **G10 KC (C.10.01)**, mapped to "STAAR Eng II counterclaim," and opens G10 as U1 Counterargument; it is **not** a G9 KC (G9 = C.9.01/02/03/04/05/06, none counterclaim); the G9 U4 gate is defined as a single-source argument-OR-informational essay; and the SRSD design record already **deferred** counterargument out of the G9 spine ("say-show-so-what only"). The l27 requirement contradicts the course's own architecture.
- **Fix:** remove the counterclaim move from the l27 gate task line, one-reminder, checklist item, sentence frame, and the PP100 pass criteria. The gate stays coherent as the G9 argument spine (claim → attributed evidence per reason → warrant → upshot). Within-KC (C904); no renderer/pipeline/gate-structure change; lesson id stable.
...
| **S2** | **DROP counterclaim from l27 gate + PP100** (stays G10; decided) | **high — decided** | within-KC C904 |
